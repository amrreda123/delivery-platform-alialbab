...
@php
    ...
    $themes = [
        ['borderColor' => 'hover:border-purple-300', 'shadowColor' => 'hover:shadow-[0_20px_40px_-15px_rgba(139,92,246,0.3)]', 'iconBg' => 'from-[#8B5CF6] to-[#7C3AED]', 'textColor' => 'group-hover:text-purple-600 text-purple-600'],
        ['borderColor' => 'hover:border-orange-300', 'shadowColor' => 'hover:shadow-[0_20px_40px_-15px_rgba(249,115,22,0.3)]', 'iconBg' => 'from-[#F97316] to-[#EA580C]', 'textColor' => 'group-hover:text-orange-600 text-orange-600'],
        ['borderColor' => 'hover:border-teal-300', 'shadowColor' => 'hover:shadow-[0_20px_40px_-15px_rgba(20,184,166,0.3)]', 'iconBg' => 'from-[#14B8A6] to-[#0D9488]', 'textColor' => 'group-hover:text-teal-600 text-teal-600'],
        ['borderColor' => 'hover:border-blue-300', 'shadowColor' => 'hover:shadow-[0_20px_40px_-15px_rgba(59,130,246,0.3)]', 'iconBg' => 'from-[#3B82F6] to-[#2563EB]', 'textColor' => 'group-hover:text-blue-600 text-blue-600'],
        ['borderColor' => 'hover:border-yellow-300', 'shadowColor' => 'hover:shadow-[0_20px_40px_-15px_rgba(255,193,7,0.3)]', 'iconBg' => 'from-[#FFC107] to-[#F59E0B]', 'textColor' => 'group-hover:text-yellow-600 text-yellow-600'],
    ];
    ...
    $description = 'نوصلك كل اللي تحتاجه بسرعة وأمان وفي أسرع وقت ممكن.';
    $action = 'اطلب الآن';

    $isUploaded = $category->icon && str_contains($category->icon, '/');

    if ($isUploaded) {
        $iconHtml = '<div class="w-16 h-16 mb-6 rounded-2xl overflow-hidden group-hover:scale-110 transition-transform duration-500 shadow-lg border-2 border-white"><img src="' . asset('storage/' . $category->icon) . '" class="w-full h-full object-cover bg-white"></div>';
    } else {
        ...
        $iconHtml = '<div class="w-16 h-16 mb-6 rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-500 bg-gradient-to-br ' . $iconBg . '">' . $svg . '</div>';
    }

    $titleHoverColor = explode(' ', $textColor)[0] ?? 'group-hover:text-yellow-600';
...
<a href="{{ route('order.create', $category->id) }}" data-aos="fade-up" data-aos-delay="{{ $delay }}" class="group bg-white rounded-[24px] p-8 border border-gray-100 hover-lift {{ $borderColor }} {{ $shadowColor }} transition-all duration-500 cursor-pointer flex flex-col items-start text-right block">
    ...
    <div class="inline-flex items-center gap-2 {{ $actionTextColor }} font-bold mt-auto">
        {{ $action }}
