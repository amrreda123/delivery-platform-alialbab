        ...
        <form action="{{ route('admin.drivers.update', $driver->id) }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <span class="block text-sm font-bold text-[#0B1536]">{{ $driver->name }}</span>
                        </div>
                        <div>
                            <span class="block text-xs text-gray-500 mb-1">رقم الهاتف</span>
                            <span class="block text-sm font-bold text-[#0B1536]" dir="ltr">{{ $driver->phone }}</span>
                        </div>
                    </div>
                    <p class="text-xs text-gray-500 mt-3">تعديل الاسم ورقم الهاتف يتم من خلال قسم إدارة المستخدمين.</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer p-4 rounded-xl border border-gray-200 bg-gray-50/50 hover:bg-gray-50 transition-colors">
                        <input type="checkbox" name="is_available" value="1" {{ old('is_available', $driver->driverProfile?->is_available ?? true) ? 'checked' : '' }}
                               class="w-5 h-5 text-[#FFC107] bg-white border-gray-300 rounded focus:ring-[#FFC107] focus:ring-2">
                        <div>
                ...
                <div>
                    ...
                    <select name="vehicle_type" class="w-full bg-gray-50 border @error('vehicle_type') border-red-500 @else border-gray-200 @enderror text-gray-900 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#FFC107] focus:border-transparent transition-all outline-none">
                        <option value="motorcycle" {{ old('vehicle_type', $driver->driverProfile?->vehicle_type) == 'motorcycle' ? 'selected' : '' }}>موتوسيكل / دراجة نارية</option>
                        <option value="bicycle" {{ old('vehicle_type', $driver->driverProfile?->vehicle_type) == 'bicycle' ? 'selected' : '' }}>دراجة هوائية</option>
                        <option value="car" {{ old('vehicle_type', $driver->driverProfile?->vehicle_type) == 'car' ? 'selected' : '' }}>سيارة</option>
                        <option value="van" {{ old('vehicle_type', $driver->driverProfile?->vehicle_type) == 'van' ? 'selected' : '' }}>سيارة نقل / فان</option>
                    </select>
                    @error('vehicle_type')
