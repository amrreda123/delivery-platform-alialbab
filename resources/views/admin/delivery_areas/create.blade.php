        ...
        <form action="{{ route('admin.delivery-areas.store') }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="delivery_fee" value="{{ old('delivery_fee', 0) }}" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#FFC107] focus:ring-2 focus:ring-yellow-100 transition-all outline-none bg-gray-50 focus:bg-white text-sm font-semibold" placeholder="أدخل تكلفة التوصيل لهذه المنطقة">
