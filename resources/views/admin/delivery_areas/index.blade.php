...
@section('title', 'إدارة مناطق التوصيل')
@section('page-title', 'مناطق التوصيل')
@section('page-subtitle', 'إدارة المناطق المتاحة للتوصيل')

@section('content')
<div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
    <div class="flex items-center justify-between mb-6">
        <h3 class="font-black text-[#0B1536] text-lg">قائمة المناطق</h3>
        <a href="{{ route('admin.delivery-areas.create') }}" class="bg-[#FFC107] hover:bg-yellow-400 text-[#0B1536] px-5 py-2.5 rounded-xl font-black text-sm transition-all duration-200 shadow-lg shadow-yellow-500/30 flex items-center gap-2">
                ...
                <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td class="p-4">
                        <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <form id="delete-area-{{ $area->id }}" action="{{ route('admin.delivery-areas.destroy', $area) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="confirmDelete('delete-area-{{ $area->id }}', '{{ addslashes($area->name) }}')" class="w-8 h-8 rounded-lg bg-red-50 text-red-600 hover:bg-red-600 hover:text-white flex items-center justify-center transition-colors">
